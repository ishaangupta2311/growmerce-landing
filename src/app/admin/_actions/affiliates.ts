...
export async function createPayout(_state: FormState, form: FormData): Promise<FormState> {
  ...
  redirect(
    `/admin/affiliates/payouts?recorded=${result.amountCents}` +
...
/**
 * Every admin view that shows a partner or what they are owed — the admin
 * overview carries the affiliate figures too — plus the partner's own
 * dashboard.
 *
...
function revalidateAdmin(partnerId: number) {
  ...
  revalidatePath(`/admin/affiliates/partners/${partnerId}`);
