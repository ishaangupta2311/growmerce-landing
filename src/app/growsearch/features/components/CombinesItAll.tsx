...
/**
 * "Growmerce combines it all" — the five layers between a supplier's raw files
 * and what a shopper sees, drawn as an isometric stack that builds bottom-up.
 *
 * Built in CSS 3D rather than shipped as a GIF: it stays sharp at any size,
 * the labels stay real text, hovering a layer raises it, and a reduced-motion
 * visitor gets the finished stack without the loop.
 */
...
/* Bottom of the stack first — the order they arrive in. Cool and unsorted at
...
/* The loose files that have not been through the stack yet. Positions are in
   the isometric plane, so they scatter around the base rather than on screen
   axes. */
const CUBES = [
  { x: -235, y: 20, s: 30, d: 0 },
    ...
    <Reveal className="mx-auto mt-16 max-w-[1370px] px-6">
      ...
      >
        ...
        <p className="mt-2 max-w-[54ch] text-[15px] leading-relaxed text-body-mute sm:text-base">
          ...
          storefront &mdash; one layer at a time.
        </p>

        <div className="mt-6 grid items-center gap-8 lg:grid-cols-[1fr_340px] lg:gap-4">
          {/* The stack. Hidden under lg, where there is not enough width for
              the isometric footprint to read as anything but clutter. */}
          <div
            aria-hidden
          ...
          >
            <div
              ...
              style={{
                transform:
                  "translate(-50%, -50%) rotateX(56deg) rotateZ(45deg)",
                      ...
                      style={
                        {
                          ...
                          boxShadow: extrude(layer.edge, THICKNESS),
                          "--slab-delay": `${i * 130}ms`,
                        } as CSSProperties
                      }
                    />
          ...
          {/* Legend, and the whole thing's accessible form: top layer first,
